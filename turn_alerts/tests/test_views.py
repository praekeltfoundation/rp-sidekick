...
class TurnAlertsRoundTripTests(APITestCase):
    def test_contacts_payload(self):
        user = get_user_model().objects.create_user("test")
        self.client.force_authenticate(user)
        response = self.client.post(
            reverse("turn-messages"),
            json.dumps(
                {
                    "contacts": [
...
class TurnAlertsValidationTests(APITestCase):
    def test_status_payload(self):
        user = get_user_model().objects.create_user("test")
        self.client.force_authenticate(user)
        response = self.client.post(
        ...
        expected = {"statuses": [{"recipient_id": ["This field is required."]}]}
